...
function handleCredentialResponse(response) {
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            userPicture = data.picture; // očekuješ da backend vrati sliku iz Google profila
...
document.getElementById('save-job-button').addEventListener('click', () => {
    ...
    const url = `${BACKEND_URL}/subcategories/${currentJobId}`;
